...
class wav_record
{
	...
	{
		strcpy(cmd_arec_1, "arecord -f S16_LE -c 1 -r 44100 -d 2 -D hw:1,0 ../wavFiles/");		
	...
	{	
		strcpy(cmd_compress,"bzip2 ../wavFiles/");
		sprintf(cmd_compress_complete,"%s%s",cmd_compress,wav_Name); 
		system(cmd_compress_complete);
		return true;
	}

	bool transferToFDB(char *file_Name)
	{	

		strcpy(cmd_sshpass_1, "scp -i /home/pi/ricktam_aws.pem ");
		strcpy(cmd_sshpass_2, " ec2-user@ec2-52-35-198-120.us-west-2.compute.amazonaws.com:/home/ec2-user/Raspberry1/Received_Files/");
		sprintf(cmd_sshpass_complete,"%s%s%s",cmd_sshpass_1,file_Name,cmd_sshpass_2);
cout<<cmd_sshpass_complete<<endl;
