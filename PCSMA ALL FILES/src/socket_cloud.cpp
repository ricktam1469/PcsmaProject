#include <stdio.h> include <iostream> include <stdio.h> include 
#<string.h> include <string> include <sys/socket.h> include <sys/time.h> 
#include <arpa/inet.h> include <netdb.h> include <unistd.h> include 
#<fstream> include <cstdlib> include <ctime> include <iomanip> include 
#<limits> include <complex> include <time.h> include <cstring>
using namespace std;
	char *board;
    	int port,freq;
...
/**
    TCP Client class */ class tcp_client { private:
	int sock;
	std::string address;
    	int port;
...
0) < 0)
	...
	*/
	...
	void generateOPCFormat()
	{
		...
		time(&t);
		timeptr=localtime(&t);
		sprintf(date,"%04d:%02d:%02d",timeptr->tm_year+1900, 
timeptr->tm_mon +1,timeptr->tm_mday );
		sprintf(time_str,"%02d:%02d:%02d:%02ld",timeptr->tm_hour, 
timeptr->tm_min,timeptr->tm_sec,start.tv_usec/1000 );
		sprintf(Packet_Frame,"#S,%s,%s,%s,20,%s;\n",board,date,time_str,val);
		//cout<<"INSIDE SENDING DATA: "<<Rcvd_String<<endl;
		sprintf(val,"%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g",2.3,1.2,5.6,7.8,9.0,2.4,8.1,10.1,11.5,13.6,14.9,15.7,7.1);
		gettimeofday(&start, NULL);
		time(&t);
		timeptr=localtime(&t);
		sprintf(date,"%04d:%02d:%02d",timeptr->tm_year+1900, 
timeptr->tm_mon +1,timeptr->tm_mday );
		sprintf(time_str,"%02d:%02d:%02d:%02ld",timeptr->tm_hour, 
timeptr->tm_min,timeptr->tm_sec,start.tv_usec/1000 );
		sprintf(Packet_Frame,"#S,%s,%s,%s,52,%s;\n",board,date,time_str,val);
	}
	
	void Sending_Data(string Rcvd_String)
	{
		generateOPCFormat();
	...
	/*void masterSwitching()
	{
		if(receivedData==plc_receive)
		{
			...
			{
				string 
new_server=plc_receive.substr(0,position);
